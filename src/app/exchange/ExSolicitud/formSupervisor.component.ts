...
@Component({
    selector: 'solicitudSupervisorForm',
    ...
    templateUrl: 'formSupervisor.html',
    encapsulation: ViewEncapsulation.None
})
export class ExSolicitudSupervisorFormComponent extends GeBaseComponent implements OnInit{
    ...
    public sitCuentaEmpDestino: SelectItem[];
    ...
    public flagBtnValidar: boolean;
    public flagBtnEjecutar: boolean;
    ...
    constructor(router: Router,
        ...
        super('ExSolicitudSupervisorFormComponent', router, configuration,authService);
    }
    ...
    public ngOnInit(){
        ...
        this.solicitudSocioForm = this.fb.group({
            ...
            'cEmpDestinoDto': new FormControl('', Validators.nullValidator),
            'numVoucherDestino': new FormControl('', Validators.nullValidator),
    ...
    onBeforeShow(event) {
        ...
        if(this.accion == 2 || this.accion == 4){
            ...
            this.solicitudSocioForm.controls["cEmpDestinoDto"].setValue(null);
            if (this.dto.etapa == this.geGenericConst.ETAPASOL_COD_EJECUCION && this.dto.estado == this.geGenericConst.ESTSOL_COD_EJECUTADA) {
                this.sitCuentaEmpDestino = [];
                this.sitCuentaEmpDestino.push({ label: this.dto.cEmpDestinoDto.cuenta + " - (CI: " +  this.dto.cEmpDestinoDto.cuentaInter + ")", value: this.dto.cEmpDestinoDto });
                this.solicitudSocioForm.controls["cEmpDestinoDto"].setValue(this.dto.cEmpDestinoDto);
                this.solicitudSocioForm.controls["numVoucherDestino"].setValue(this.dto.numVoucherDestino);
            }
        ...
        this.gestionarBtns();
        if(this.flagBtnEjecutar) {
            //Verificamos si la solicitud ya se encuentra apta para ser ejecutada
            if (this.dto.etapa == this.geGenericConst.ETAPASOL_COD_VALIDACION && this.dto.estado == this.geGenericConst.ESTSOL_COD_VALIDADA) {
                this.solicitudSocioForm.controls["cEmpDestinoDto"].setValidators(Validators.required);
                this.solicitudSocioForm.controls["numVoucherDestino"].setValidators(Validators.compose([Validators.required, Validators.maxLength(200)]));
                this.solicitudSocioForm.controls["cEmpDestinoDto"].updateValueAndValidity();
                this.solicitudSocioForm.controls["numVoucherDestino"].updateValueAndValidity();
                this.obtenerCuentaEmpresaDestino();
            }
        }
    }
    ...
    private gestionarBtns(){
        ...
        this.flagBtnValidar = false;
        this.flagBtnEjecutar = false;
        ...
        if(this.accion == 1){
            ...
            if(((this.dto.etapa == this.geGenericConst.ETAPASOL_COD_REVISION && this.dto.estado == this.geGenericConst.ESTSOL_COD_REVISADA)) || (this.dto.etapa == this.geGenericConst.ETAPASOL_COD_VALIDACION && (this.dto.estado == this.geGenericConst.ESTSOL_COD_OBSERVADA || this.dto.estado == this.geGenericConst.ESTSOL_COD_PENDIENTE))){
                this.flagBtnObservar = true;
                this.flagBtnValidar = true;
            }

            if(this.dto.etapa == this.geGenericConst.ETAPASOL_COD_VALIDACION && this.dto.estado == this.geGenericConst.ESTSOL_COD_VALIDADA){
                this.flagBtnEjecutar = true;
                this.flagBtnGuardar = false;
    ...
    public formValidarSolicitud(){
        ...
        var solicitudes: ExSolicitudDto[] = [];
        solicitudes.push(this.dto);
        this.validarSolicitudes(solicitudes);
    }

    public formEjecutarSolicitud(){
        this.dto = this.solicitudSocioForm.value;
        this.recuperarValorCalculados();
        this.ejecutarSolicitud(this.dto);
    }
    ...
    public observarSolicitudes(solicitudes: ExSolicitudDto[]){
        this.confirmationService.confirm({ //Mensaje de confirmación de guardar el registro
            ...
            accept: () => {
                this.dto.cEmpDestinoDto = null;
                this.service
                    .observarEnValidacion(solicitudes)
                    .subscribe(
                        (response: GeMensajeHttpDto) => {
                            this.displayDialog = false;
                            this.accion = 0;
    ...
    public validarSolicitudes(solicitudes: ExSolicitudDto[]){
        this.confirmationService.confirm({ //Mensaje de confirmación de guardar el registro
    ...
    public ejecutarSolicitud(solicitud: ExSolicitudDto){
        this.confirmationService.confirm({ //Mensaje de confirmación de guardar el registro
            message: 'Está seguro que desea ejecutar el registro?',
            header: 'Confirmacion',
            accept: () => {
                this.service
                    .ejecutar(solicitud)
    ...
    public getSocioPorUsuario() {
        ...
        this.socNegocioService
            .getUsuario(login)
            .subscribe(
                (response: GeSocioNegocioDto) => { this.solicitudSocioForm.controls["socioNegocioDto"].setValue(response);},
                error => {
                    this.mostrarError(error);
                }
            );
    ...
    public obtenerCuentaEmpresaDestino(){
        var moneda: any = this.solicitudSocioForm.controls['monedaDestinoDto'].value;

        if(moneda){
            if (moneda.id != this.geGenericConst.filtroSeleccionar.codigo){
                this.sitCuentaEmpDestino = this.cEmpresaService.getCuentaEmpresa2(moneda.id, this.solicitudSocioForm.controls['cEmpDestinoDto']);
            }else
                this.sitCuentaEmpDestino = [];
        }else
            this.sitCuentaEmpDestino = [];
