...
import { SelectItem, ConfirmationService, InputMaskModule, FieldsetModule } from 'primeng/primeng';
...
@Component({
    ...
    templateUrl: 'formSocio.html',
    encapsulation: ViewEncapsulation.None,
    styles: [
        '.ui-dialog .ui-dialog-content{overflow: inherit !important;} body .ui-widget-content{overflow: inherit;}'
    ]
})
export class ExSolicitudSocioFormComponent extends GeBaseComponent implements OnInit{
    ...
    @Input() //Variable de entrada cuando se invocara a este componente
    dto: ExSolicitudDto = new ExSolicitudDto();
    ...
    public flagBtnLevantObs: boolean;
    ...
    constructor(router: Router,
       ...
       super('ExSolicitudSocioFormComponent', router, configuration,authService);
    }
    ...
    private gestionarBtns(){
        ...
        this.flagBtnLevantObs = false;
        this.flagInfoEjecutar = false;
        if(this.accion == 1){
            this.flagBtnGuardar = true;
        }else{
            if(this.dto.etapa == this.geGenericConst.ETAPASOL_COD_REGISTRO && this.dto.estado == this.geGenericConst.ESTSOL_COD_PENDIENTE){
                this.flagBtnGuardar = true;
            }
            ...
            if(this.dto.etapa == this.geGenericConst.ETAPASOL_COD_REVISION && this.dto.estado == this.geGenericConst.ESTSOL_COD_OBSERVADA){
                this.flagBtnLevantObs = true;
                this.flagBtnGuardar = false;
            }

            if(this.dto.etapa == this.geGenericConst.ETAPASOL_COD_VALIDACION && this.dto.estado == this.geGenericConst.ESTSOL_COD_OBSERVADA){
                this.flagBtnLevantObs = true;
                this.flagBtnGuardar = false;
            }
        }
                ...
                if(this.accion == 1 || this.accion == 4){
                    ...
                    .subscribe(
                        (response: GeMensajeHttpDto) => {
                            console.log("respuesta del server  : " + response.codigoHttp);
                            if(response.codigoHttp == this.geGenericConst.codStatusOK){
                                this.accion = 2;
                            ...
                            }else if(response.codigoHttp == this.geGenericConst.codErrorValidacion200){
                                this.msgsPrincipal.push({ severity: 'error', summary: 'Validación del negocio', detail: response.mensajeUsuario });
                                this.displayDialog = true;
                            }else if(response.codigoHttp == this.geGenericConst.codErrorValidacionNoBlock200){
                                this.confirmationService.confirm({
                                    message: response.mensajeUsuario,
                                    header: 'Confirmación',
                                    accept: () => {
                                        this.dto.flagConfirm = true;
                                        this.saveAlt();
                                    },
                                    reject: () => {
                                        //No realiza nada
                    ...
                    this.service
                    .actualizarPorSocio(this.dto)
                    .subscribe(
                        (response: ExTipoCambioDto) => {
                            this.displayDialog = false;
                            this.accion = 0;
                            this.respuesta.emit({ severity: 'success', summary: 'Mensaje de Conformidad', detail: 'Entidad grabada correctamente!', dto: this.dto });
                        },
                        error => {
                            let errorHttpDto:GeMensajeHttpDto = error;
                            this.msgsPrincipal.push({ severity: 'error', summary: errorHttpDto.resumenHttp, detail: errorHttpDto.mensajeSistemaHttp });
    ...
    public formLevantarObs(){
        this.dto = this.solicitudSocioForm.value;
        this.recuperarValorCalculados();
        this.levatObsSolicitud(this.dto);
    }
    ...
    public levatObsSolicitud(solicitud: ExSolicitudDto){
        this.confirmationService.confirm({ //Mensaje de confirmación de guardar el registro
            message: 'Está seguro que desea levantar la observación del registro?',
            header: 'Confirmacion',
            accept: () => {
                this.service
                    .levantarObs(solicitud)
                    .subscribe(
                        (response: GeMensajeHttpDto) => {
                            this.displayDialog = false;
                            this.accion = 0;
